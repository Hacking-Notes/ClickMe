...
function addIframeElement(stepContainer) {
    ...
    const containerWidth = iframeSection ? iframeSection.offsetWidth : 800;
    const containerHeight = iframeSection ? iframeSection.offsetHeight : 600;
    
    // Calculate default position (50% of container)
    const defaultLeft = 50; // Use percentage instead of pixels
    const defaultTop = 50; // Use percentage instead of pixels
    ...
    iframeElement.innerHTML = `
        ...
        <div class="element-content hidden">
            <div class="iframe-settings">
                ...
                <div class="iframe-dimension">
                    <label>Left:</label>
                    <input type="number" class="iframe-left" value="${defaultLeft}" min="0" max="100">
                    <span>%</span>
                </div>
                <div class="iframe-dimension">
                    <label>Top:</label>
                    <input type="number" class="iframe-top" value="${defaultTop}" min="0" max="100">
                    <span>%</span>
                </div>
            </div>
        </div>
...
function updateStepContent() {
    ...
    steps.forEach((step, index) => {
        ...
        if (iframeElement) {
            ...
            const left = iframeElement.querySelector('.iframe-left')?.value || '0';
            const top = iframeElement.querySelector('.iframe-top')?.value || '0';
            stepsContent[stepKey].iframe = { 
                width: width, 
                height: height,
                left: left,
                top: top
            ...
            if (stepData) {
                ...
                if (stepData.iframe) {
                    ...
                    if (iframeElement) {
                        ...
                        const leftInput = iframeElement.querySelector('.iframe-left');
                        const topInput = iframeElement.querySelector('.iframe-top');
                        ...
                        if (leftInput && stepData.iframe.left) {
                            leftInput.value = stepData.iframe.left;
                        }

                        if (topInput && stepData.iframe.top) {
                            topInput.value = stepData.iframe.top;
                        }
                    }
                }
                            ...
                            <div class="button-type-settings captcha-puzzle-button-settings" ${settings.type !== 'captcha-puzzle' ? 'style="display: none;"' : ''}>
                                <div class="setting-row">
                                    ...
                                    <div class="position-inputs">
                                        <div class="position-input">
                                            <span>Left:</span>
                                            <input type="number" class="captcha-puzzle-left" value="${settings.left || 100}" min="0">
                                            <span>%</span>
                                        </div>
                                        <div class="position-input">
                                            <span>Top:</span>
                                            <input type="number" class="captcha-puzzle-top" value="${settings.top || 100}" min="0">
                                            <span>%</span>
                                        </div>
                                    </div>
                                </div>
...
function updateIframeToMatchSelectedStep(stepNumber) {
    ...
    if (!mainIframe || !iframeSection) return;
    ...
    // Get the width, height, left, and top inputs
    const widthInput = iframeElement.querySelector('.iframe-width');
    const heightInput = iframeElement.querySelector('.iframe-height');
    const leftInput = iframeElement.querySelector('.iframe-left');
    const topInput = iframeElement.querySelector('.iframe-top');
    ...
    if (widthInput && heightInput) {
        ...
        // Get position values as percentages, use 50% as default for centering
        const leftPercent = leftInput ? parseInt(leftInput.value) || 50 : 50;
        const topPercent = topInput ? parseInt(topInput.value) || 50 : 50;
        
        // Ensure iframe-section has proper positioning context
        iframeSection.style.position = 'relative';
        
        // Position the main container (outer container) using percentages
        if (mainContainer) {
        ...
        // Position the iframe directly
        mainIframe.style.width = `${width}px`;
        mainIframe.style.height = `${height}px`;
        mainIframe.style.position = 'absolute';
        mainIframe.style.left = `${leftPercent}%`;
        mainIframe.style.top = `${topPercent}%`;
        mainIframe.style.transform = 'translate(-50%, -50%)'; // Center the iframe at the percentage point
        mainIframe.style.zIndex = '3';
        
        // Position the button container to match the iframe
        if (buttonContainer) {
            ...
            buttonContainer.style.transform = 'translate(-50%, -50%)'; // Center the container at the percentage point
            buttonContainer.style.zIndex = '4';
        }
        
        console.log(`Updated iframe dimensions to ${width}x${height} at position (${leftPercent}%, ${topPercent}%)`);
    }
}
...
function setupButtonContainerCSS() {
    ...
    style.textContent = `
        ...
        /* Captcha puzzle button positioning */
        #captcha-puzzle-button {
            position: absolute !important;
...
function updateButtonPreview(buttonType, input) {
    ...
    if (buttonType === 'normal') {
        ...
        if (className === 'captcha-puzzle-left') {
            ...
            captchaPuzzle.style.transform = 'translate(-50%, -50%)';
            console.log(`Updated captcha puzzle left position to ${value}%`);
        }
        else if (className === 'captcha-puzzle-top') {
            captchaPuzzle.style.top = `${value}%`;
            captchaPuzzle.style.transform = 'translate(-50%, -50%)';
            console.log(`Updated captcha puzzle top position to ${value}%`);
        }
        ...
        if (buttonTypeSelect) {
            ...
            if (buttonType === 'normal') {
                ...
                if (captchaPuzzle) {
                    ...
                    const left = buttonElement.querySelector('.captcha-puzzle-left')?.value || '50';
                    const top = buttonElement.querySelector('.captcha-puzzle-top')?.value || '50';
...
function centerButtonInIframe(stepContainer, buttonElement) {
    ...
    const centerX = 100; // Use percentage instead of pixels
    const centerY = 100; // Use percentage instead of pixels
